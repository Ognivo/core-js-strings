...
function isPalindrome(str) {
  const nStr = str.replace(/[^a-zA-Z0-9]/g, '').toLocaleLowerCase();
...
function findLongestWord(sentence) {
  ...
  lWord = words.reduce((longest, current) => {
    return current.length > longest.length ? current : longest;
  });
  /** Параметр longest представляет аккумулятор,
    а current представляет текущее обрабатываемое слово */
  return lWord;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(
    /[a-z]/gi,
    (letter) => alphab[alphab.indexOf(letter) + 13]
  );
}
